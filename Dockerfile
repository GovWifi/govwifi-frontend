...
RUN wget https://github.com/FreeRADIUS/freeradius-server/releases/download/release_3_2_2/freeradius-server-3.4.5.tar.gz \
    && tar xzvf freeradius-server-3.4.5.tar.gz \
    && cd freeradius-server-3.4.5 \
    && ./configure CPPFLAGS=-DX509_V_FLAG_PARTIAL_CHAIN=1 --sysconfdir=/etc \
    && make \
    && make install
RUN rm -rf ./freeradius-server-3.4.5
...
RUN curl https://github.com/bvantagelimited/freeradius_exporter/releases/download/3.4.5/freeradius_exporter-0.1.6-amd64.tar.gz --location --output freeradius_exporter.tar.gz \
 && echo "38bf31e6a35e2afe0959e9f6eb90b1beb6b84e32d02448ee1005093ee322f173  freeradius_exporter.tar.gz" > checksums \
 && sha256sum -c checksums \
 && rm checksums \
 && tar -xzvf freeradius_exporter.tar.gz --strip-components=1 freeradius_exporter-3.4.5-amd64/freeradius_exporter \
